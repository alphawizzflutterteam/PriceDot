  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        body: videoModel == null || videoModel == ""
            ? Center(child: CircularProgressIndicator(color: AppColors.primary))
            : videoModel?.content?.length == 0
                ? Text("No our activity found!!!")
  ...
  Future<void> getVideo() async {
    apiBaseHelper.postAPICall2(getVideoAPI).then((getData) {
      ...
      for (var i = 0; i < videoModel!.content!.length; i++) {
        _vController.add(VideoPlayerController.network(
            videoModel!.content![i].videoLink.toString())
          ..initialize().then((value) {
            setState(() {});
          }));
        isPlayed.add(false);
        print(
            "video controller length ${_vController.length}----------${_vController}");
      }
